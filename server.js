...
function validateForm(formData) {
  ...
  } else if (formData.nome.length > MAX_LENGTH.nome) {
    errors.push(`Il campo Nome non può superare ${MAX_LENGTH.nome} caratteri`);
  }

  if (!formData.email || formData.email.trim() === '') {
    errors.push('Il campo Email è obbligatorio');
  } else if (formData.email.length > MAX_LENGTH.email) {
    errors.push(`Il campo Email non può superare ${MAX_LENGTH.email} caratteri`);
  } else {
  ...
  if (formData.telefono && formData.telefono.length > MAX_LENGTH.telefono) {
    errors.push(`Il campo Telefono non può superare ${MAX_LENGTH.telefono} caratteri`);
  }

  if (!formData.messaggio || formData.messaggio.trim() === '') {
    errors.push('Il campo Messaggio è obbligatorio');
  } else if (formData.messaggio.length > MAX_LENGTH.messaggio) {
    errors.push(`Il campo Messaggio non può superare ${MAX_LENGTH.messaggio} caratteri`);
  }
...
app.post('/submit-form', formLimiter, async (req, res) => {
  try {
    ...
    console.error('Errore durante l\'elaborazione del form:', error.message);
    res.status(500).json({
      success: false,
